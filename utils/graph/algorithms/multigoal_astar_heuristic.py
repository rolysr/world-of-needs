...
def multigoal_astar_heuristic(destination_nodes, graph, human_agent, destination_agents, number_of_needs):
    """
        Calculate a node heuristic function to a node on a graph with multiple goals
    """
    destination_agents_quality = get_destination_agents_quality(human_agent, destination_agents, number_of_needs) # get destinations qualities
    heuristic = { node: inf for node in graph.nodes } # initialize heuristic
    
    for destination_agent in destination_agents:
        destination_node = destination_nodes[destination_agent]
        ...
        for node in heuristic.keys(): # update best value for node heuristic
            destination_agent_quality = destination_agents_quality[destination_agent] # get destination agent quality
            heuristic[node] = min(heuristic[node], dist[node] * destination_agent_quality * 0.5) # calculate best minimum value for all destination agent's qualities 
